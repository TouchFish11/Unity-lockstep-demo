...
{
    /// <summary>
    /// AssetBundle资源请求任务封装类
    /// 支持异步等待、取消操作，泛型约束为Unity引擎对象
    /// </summary>
    /// <typeparam name="T">要加载的资源类型，继承自UnityEngine.Object</typeparam>
    public class AssetBundleRequestTask<T> : IPoolData where T : Object
    {
        // AssetBundle原生请求对象
        ...
        public void Init(AssetBundleRequest request, CancellationToken token = default)
        {
            _abr = request;
            // 注册原生请求完成的回调
            ...
            if (_cancellationToken.CanBeCanceled)
            {
                ...
                {
                    var task = (AssetBundleRequestTask<T>)state;
