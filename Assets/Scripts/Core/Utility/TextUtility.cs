...
namespace Core.Utility
{
    ...
    public static class TextUtility
    {
        ...
        public static string[] Split(string str, int type)
        {
            ...
            if (str == "")
            {
                Logger.LogWarning(ELogTags.Utility, "待分割的字符串长度为0");
            ...
            switch (type)
            {
                ...
                default:
                    // 无效分隔符类型告警
                    Logger.LogError(ELogTags.Utility, $"未提供该类型的分割方式，当前type值为，{type}。范围：1-7");
                    return Array.Empty<string>();
            }
        }
        ...
        public static int[] SplitToIntArr(string str, int type)
        {
            ...
            if (newStr.Length == 0)
            {
                Logger.LogWarning(ELogTags.Utility, "待转换的字符串数组长度为0，无法转为int数组");
            ...
            return Array.ConvertAll(newStr, (singleStr) =>
            {
                ...
                // 转换失败告警
                Logger.LogWarning(ELogTags.Utility, $"字符串「{singleStr}」转换int失败，返回默认值0");
                return 0;
            });
        }
        ...
        public static float[] SplitTofloatArr(string str, int type)
        {
            ...
            if (newStr.Length == 0)
            {
                Logger.LogWarning(ELogTags.Utility, "待转换的字符串数组长度为0，无法转为float数组");
            ...
            return Array.ConvertAll(newStr, (singleStr) =>
            {
                ...
                // 转换失败告警
                Logger.LogWarning(ELogTags.Utility, $"字符串「{singleStr}」转换float失败，返回默认值0.0f");
                return 0;
            });
        }
        ...
        public static void SplitMultiple(string str, int firstSplitCharType, int secondSplitCharType, UnityAction<int, int> callBack)
        {
            ...
            if (newStr.Length == 0)
            {
                Logger.LogWarning(ELogTags.Utility, "第一层分割后的字符串数组长度为0，终止双层分割");
                return;
            }
